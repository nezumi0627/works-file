"""Works Mobile APIクライアント - ボイスメッセージ送信用."""
...
class WorksVoiceAPI:
    ...
    Works Mobile APIを使用してボイスメッセージのアップロードを行うクラス.
    """
    ...
    def upload_file(
        ...
        Note:
            ...
            拡張子が対応外のファイルである可能性があります。
        """
        uploader = VoiceUploader(file_data)
        ...
        form.append("Content-Type: audio/m4a")
        form.append("")
...
def main() -> None:
    ...
    api = WorksVoiceAPI(channel_no=channel_no, caller_no=caller_no)

    # ファイルパスを修正
    file_path = os.path.join(os.path.dirname(__file__), "files", "voice.m4a")
