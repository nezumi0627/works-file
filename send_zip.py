...
class WorksAPI:
    """Works Mobile APIクライアント.

    Works Mobile APIを使用してZIPファイルのアップロードを行うクラス.
    """
    ...
    def upload_file(
        ...
        """
        uploader = FileUploader(file_data)
        ...
        boundary = "----WebKitFormBoundaryipK1wBqIkziAwC3M"
        extras = {
        ...
        form.append("Content-Type: application/x-zip-compressed")
        form.append("")
...
def main() -> None:
    ...
    api = WorksAPI(channel_no=channel_no, caller_no=caller_no)

    # ファイルパスを修正
    file_path = os.path.join(os.path.dirname(__file__), "files", "test.zip")
