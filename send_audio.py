"""Works Mobile APIクライアント - 音声アップロード用."""
...
class WorksAudioAPI:
    ...
    Works Mobile APIを使用して音声ファイルのアップロードを行うクラス.
    """
    ...
    def upload_file(
        ...
        Note:
            ...
            拡張子が対応外のファイルである可能性があります。
        """
        uploader = AudioUploader(file_data)
        ...
        boundary = "----WebKitFormBoundaryufAEavRICbCNszLW"
        extras = {
        ...
        form.append("Content-Type: audio/x-m4a")
        form.append("")
...
def main() -> None:
    ...
    api = WorksAudioAPI(channel_no=channel_no, caller_no=caller_no)

    # ファイルパスを修正
    file_path = os.path.join(os.path.dirname(__file__), "files", "audio.m4a")
